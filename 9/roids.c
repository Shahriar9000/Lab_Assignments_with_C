...
draw_triangle( ship0.x + dx, ship0.y + dy,
  ...
  */
	/*if(ship->x+dx < 0)
	{
		draw_triangle( ship->x + dx + 1, ship->y + dy,ship->x + lx + 1, 
				ship->y + ly, ship->x + rx + 1,
				 ship->y + ry,shipcolor );
	}

	else if(ship->y+dy < 0)
	{
		draw_triangle( ship->x + dx, ship->y + dy + 1,ship->x + lx, ship->y + ly + 1,
				ship->x + rx, ship->y + ry + 1,shipcolor );
	}
	
	else if(ship->x+dx > 1)
	{
		draw_triangle( ship->x + dx - 1, ship->y + dy,ship->x + lx - 1, ship->y + ly,
				ship->x + rx - 1, ship->y + ry,shipcolor );
	}
	...
	if( ship->thrust )
	{
		...
		{
			draw_triangle( ship->x - dx + 1, ship->y - dy,ship->x + lx + 1, ship->y + ly,
					ship->x + rx + 1, ship->y + ry,thrustcolor );
		}
		else if(ship->y-dy < 0)
		{
			draw_triangle( ship->x - dx, ship->y - dy + 1,ship->x + lx, ship->y + ly + 1,
					ship->x + rx, ship->y + ry + 1,thrustcolor );
		}
		else if(ship->x+dx > 0)
		{
			draw_triangle( ship->x - dx - 1, ship->y - dy,ship->x + lx - 1, ship->y + ly,
...
float x1,y1,x2,y2;
x1=roid->x-roid->width/2.0;
x2=roid->x+roid->width/2.0;
y1=roid->y-roid->height/2.0;
y2=roid->y+roid->height/2.0;

if(x2>1 && y2>1)
{
	draw_rectangle(x1,y1,1,1, roid->color); 
	draw_rectangle(x1,0,1,y2-1,roid->color); 
	draw_rectangle(0,0,x2-1,y2-1, roid->color);
	draw_rectangle(0,y1,x2-1,1, roid->color);
}
 if (x1<0 && y2>1)
{
	draw_rectangle(0,y1,x2,y2-1, roid->color);
	draw_rectangle(1+x1,y1,1,1, roid->color);
	draw_rectangle(0,0,x2,y2-1, roid->color);
	draw_rectangle(1+x1,1,x2,y2-1, roid->color);
}
if (x1<0 && y1<0)
{
	draw_rectangle(0,0,x2,y2, roid->color);
	draw_rectangle(0,1+y1,x2,1, roid->color);
	draw_rectangle(1+x1,0,1,y2, roid->color);
	draw_rectangle(1+x1,1-y1,1,1, roid->color);
}
 if (y1<0 && x2>1)
{

	draw_rectangle(x1,0,1,y2, roid->color);
	draw_rectangle(0,0,1-x2,y2, roid->color);
	draw_rectangle(0,1+y1,1-x2,1, roid->color);
	draw_rectangle(x1,1+y1,1,1, roid->color);
}
  if (x1<0 && y1>0 && x2>0 && y2>0)
{
	draw_rectangle(0,y1,x2,y2, roid->color);
	draw_rectangle(1+x1,y1,1,y2, roid->color);
}
  if (x1>0 && y1<0 && x2>0 && y2>0)
{
	draw_rectangle(x1,0,x2,y2, roid->color);
	draw_rectangle(x1,1+y1,x2,1, roid->color);
}
 if (x1>0 && y1>0 && x2>1 && y2>0)
{	
	draw_rectangle(x1,y1,1,y2, roid->color);
	draw_rectangle(0,y1,x2-1,y2, roid->color);
}
/*else
	draw_rectangle(x1,y1,x2,y2, roid->color);*/
...
if(roid->x-roid->width/2.0 < 0)
	{
		draw_rectangle( 1+roid->x-roid->width/2.0, roid->y-roid->height/2.0,1, 
				roid->y+roid->height/2,roid->color );
	}
...
if(roid->x+roid->width/2.0 > 1)
	{
		draw_rectangle( 0, roid->y-roid->height/2.0,roid->x+roid->width/2.0-1, 
				roid->y+roid->height/2,roid->color );
	}
if(roid->y+roid->height/2.0 > 1)
	{
		draw_rectangle( roid->x-roid->width/2.0, 0,roid->x+roid->width/2.0, 
				roid->y+roid->height/2-1,roid->color );
	}
